...
状态：Runtime、隔离 CLI Builder、User Run 的已校验 bundle 物化，以及公共 broker 协议与 Runtime 客户端已实现；模型 Runtime 不挂载 CLI bundle，受 Egress Gate 保护的命令进程、broker socket 容器契约与细粒度 Egress 控制器已实现，专用 Connector Container 绑定与 User Action Wait 尚未接入；部署前必须重新构建并记录 RepoDigest
