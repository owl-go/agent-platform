...
- Refresh failure retains the last successful catalog and shows its time and the new error. Models removed by a provider become unavailable for new selection but remain identifiable in history.
- Provider management lists the complete imported catalog. Session, Runtime, and Workflow selectors show Agent/text-generation models; an unclassified model remains selectable with an unverified warning.
