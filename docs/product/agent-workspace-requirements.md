...
- Provider management lists the resulting models and provides both Refresh Models and Add Model actions. A fallback catalog is not presented as a provider error, while a connection with neither discovered nor default models shows the discovery failure and still permits manual model entry.
- Provider Models have an identifier and optional display name but no model-type classification. Every available model appears in Session, Runtime, and Workflow selectors, subject only to Runtime Model Compatibility derived from the connection's Model API Protocols.
