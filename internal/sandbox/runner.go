...
func (r *DockerRunner) Create(ctx context.Context, spec CreateSpec) (Container, error) {
	...
	args := []string{
		...
		"--mount", "type=volume,src=" + spec.WorkspaceVolume + ",dst=/workspace,rw",
		"--mount", "type=bind,src=" + spec.CredentialDirectory + ",dst=/run/agent-credentials,ro",
		"--tmpfs", "/tmp:rw,noexec,nosuid,nodev,size=" + strconv.FormatInt(spec.Limits.TempBytes, 10),
...
func (r *DockerRunner) validateInspection(inspection Inspection) error {
	...
	if !baseIsolation || !resourceIsolation || !securityIsolation || !tmpIsolation || !r.validMounts(inspection.Mounts) {
...
func (r *DockerRunner) validMounts(mounts []Mount) bool {
	workspace := false
	credentials := false
	...
	return workspace && credentials && len(mounts) == 2
}
