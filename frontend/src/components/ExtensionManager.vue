...
<template>
  ...
  <Teleport to="body">
    ...
    <div v-if="showCLI" class="modal-layer" @click.self="showCLI = false"><form class="modal-card el-card" @submit.prevent="saveCLI"><h2>{{ t('resources.cliDefinition') }}</h2><label>{{ t('common.name') }}<input v-model="cliForm.name" required></label><label>{{ t('resources.npmPackage') }}<input v-model="cliForm.npm_package" required></label><label>{{ t('resources.exactVersion') }}<input v-model="cliForm.npm_version" required placeholder="1.2.3"></label><label>{{ t('resources.npmIntegrity') }}<input v-model="cliForm.npm_integrity" required></label><label>{{ t('resources.executable') }}<input v-model="cliForm.executable" required></label><div class="modal-actions"><el-button @click="showCLI = false">{{ t('common.cancel') }}</el-button><el-button native-type="submit" type="primary">{{ t('common.save') }}</el-button></div></form></div>
  </Teleport>
