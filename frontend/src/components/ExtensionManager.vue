...
import { platformApiKey, type CLIConnectorDefinition, type CLIConnectorDefinitionInput, type CLIConnectorEnablement, type EnvironmentVariable, type Expert, type MCPServer, type Skill } from "../api/client";
...
async function refresh() {
  try { [mcp.value, skills.value, cliDefinitions.value, cliEnablements.value, experts.value] = await Promise.all([api.listMCPServers(), api.listSkills(), api.listCLIConnectorDefinitions?.() ?? Promise.resolve([]), api.listCLIConnectorEnablements?.() ?? Promise.resolve([]), api.listExperts?.() ?? Promise.resolve([])]); notifyResources(); } catch { emit("error"); }
}
...
const affectedExpertNames = computed(() => {
  const target = pendingDelete.value;
  if (!target) return [];
  return experts.value.filter((expert) => target.kind === "mcp" ? expert.mcp_server_ids.includes(target.item.id) : expert.skill_ids.includes(target.item.id)).map((expert) => expert.name);
});
...
<template>
  ...
  <Teleport to="body">
    ...
    <div v-if="showCLI" class="modal-layer" @click.self="showCLI = false"><form class="modal-card el-card" @submit.prevent="saveCLI"><h2>{{ t('resources.cliDefinition') }}</h2><label>{{ t('common.name') }}<input v-model="cliForm.name" required></label><label>npm package<input v-model="cliForm.npm_package" required></label><label>{{ t('resources.exactVersion') }}<input v-model="cliForm.npm_version" required placeholder="1.2.3"></label><label>npm integrity<input v-model="cliForm.npm_integrity" required></label><label>{{ t('resources.executable') }}<input v-model="cliForm.executable" required></label><div class="modal-actions"><el-button @click="showCLI = false">{{ t('common.cancel') }}</el-button><el-button native-type="submit" type="primary">{{ t('common.save') }}</el-button></div></form></div>
  </Teleport>
  <ConfirmDialog :open="Boolean(pendingDelete)" :title="t('common.delete')" :message="pendingDelete ? affectedExpertNames.length ? t('resources.deleteAffected', { resource: pendingDelete.item.name, experts: affectedExpertNames.join('、') }) : t('resources.deleteUnaffected', { resource: pendingDelete.item.name }) : ''" :confirm-label="t('common.delete')" :cancel-label="t('common.cancel')" danger @cancel="pendingDelete = undefined" @confirm="confirmRemove" />
</template>
