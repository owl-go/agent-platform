...
async function refresh() { try { approvals.value = await api.listCommandApprovals(); } catch { /* The page shell owns global connectivity feedback. */ } }
...
<template>
  <aside v-if="approvals.length" class="approval-inbox" aria-live="polite">
    <article v-for="item in approvals" :key="item.id" class="approval-card">
      ...
      <select v-model="identities[item.id]" :aria-label="t('approvals.identity')"><option value="user">{{ t('approvals.user') }}</option><option value="bot">{{ t('approvals.bot') }}</option></select>
