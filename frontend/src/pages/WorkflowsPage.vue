...
<template>
  <section class="page-surface">
    <header class="page-header"><div><p class="eyebrow">02 / REPEATABLE EXECUTION</p><h1>{{ t('workflows.title') }}</h1><p>{{ t('workflows.subtitle') }}</p></div><el-space wrap><el-button @click="showDeleted = !showDeleted">{{ t('workflows.deletedRecords') }} · {{ deleted.length }}</el-button><el-button type="primary" @click="showCreate = true">＋ {{ t('workflows.new') }}</el-button></el-space></header>
    ...
    <div v-else class="workflow-grid">
      <el-card v-for="(workflow, index) in workflows" :key="workflow.id" class="workflow-card" shadow="hover" :style="{ '--delay': `${index * 45}ms` }">
        ...
        <div class="workflow-meta"><el-tag size="small" effect="plain">{{ workflow.runtime_engine ? runtimeEngineDisplayName(workflow.runtime_engine) : 'AUTO' }}</el-tag><el-tag size="small" type="info" effect="plain">{{ specialistName(workflow) }}</el-tag></div>
