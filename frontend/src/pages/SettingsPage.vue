...
const manualModel = ref({ model_id: "", display_name: "" });
...
function openManualModel(item: ModelProviderConnection) { manualConnection.value = item; manualModel.value = { model_id: "", display_name: "" }; }
...
<template>
  ...
  <div v-if="manualConnection" class="modal-layer" @click.self="manualConnection = undefined"><form class="modal-card" @submit.prevent="saveManualModel"><p class="eyebrow">{{ manualConnection.name }} / MODEL</p><h2>{{ t("settings.manualModel") }}</h2><label>{{ t("settings.modelId") }}<input v-model="manualModel.model_id" required></label><label>{{ t("common.name") }}<input v-model="manualModel.display_name"></label><div class="modal-actions"><button type="button" class="button ghost" @click="manualConnection = undefined">{{ t("common.cancel") }}</button><button class="button primary">{{ t("common.save") }}</button></div></form></div>
