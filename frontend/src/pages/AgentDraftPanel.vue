...
import {
  ...
  type DraftInput, type RepositoryBinding, type RuntimeImage,
} from "../api/client";
...
watch(selectedAgentID, () => void loadDrafts());
...
async function loadDrafts() {
  ...
  const requestedTeam = teamID.value;
  if (!agentID || !requestedTeam) { drafts.value = []; return; }
  try {
    const values = await api.listAgentDrafts(agentID, requestedTeam);
    if (selectedAgentID.value === agentID && teamID.value === requestedTeam) drafts.value = values;
...
async function saveDraft() {
  ...
  try {
    ...
    intents.delete(scope); draftModal.value = false; editingDraft.value = undefined; await loadDrafts(); notice.value = t("agentCatalog.notice.draftSaved");
  } catch (reason) {
...
async function validateDraft(draft: AgentDraft) {
  ...
  try {
    ...
    intents.delete(scope); await loadDrafts(); notice.value = t("agentCatalog.notice.validated");
  } catch (reason) { error.value = asApiError(reason); }
...
<template>
  <section class="agent-catalog" data-testid="agent-catalog">
    ...
    <div v-else class="agent-workbench">
      ...
      <section class="draft-board"><header><div><span>{{ t('agentCatalog.drafts') }}</span><h3>{{ selectedAgent?.name ?? t('agentCatalog.selectAgent') }}</h3></div><button v-if="canBuild && selectedAgent" class="secondary-action" data-testid="create-draft" @click="beginDraft()">{{ t('agentCatalog.createDraft') }}</button></header><p v-if="selectedAgent && drafts.length === 0">{{ t('agentCatalog.noDrafts') }}</p><article v-for="draft in drafts" :key="draft.id" class="draft-card" :data-testid="`draft-${draft.id}`"><div class="record-heading"><div><span>R{{ draft.revision }} · V{{ draft.version }}</span><h4>{{ t(`agentCatalog.state.${validatingDraftID === draft.id ? 'validating' : draft.state ?? 'draft'}`) }}</h4></div><em :class="draft.state === 'ready' ? 'state-production' : 'state-blocked'">{{ t(`agentCatalog.risk.${draft.release_risk ?? 'low'}`) }}</em></div><dl class="binding-facts"><div><dt>{{ t('agentCatalog.runtimeImage') }}</dt><dd>{{ draft.configuration?.runtime_image_id }}</dd></div><div><dt>{{ t('agentCatalog.configuredModel') }}</dt><dd>{{ draft.configuration?.configured_model_id }}</dd></div><div class="wide"><dt>{{ t('agentCatalog.repositoryBinding') }}</dt><dd>{{ draft.configuration?.repository_binding_id }}</dd></div></dl><ul v-if="draft.validation_report && !draft.validation_report.valid" class="validation-errors"><li v-for="(message, field) in draft.validation_report.errors" :key="field"><code>{{ field }}</code><span>{{ message }}</span></li></ul><footer v-if="canBuild"><button class="record-action" :disabled="saving" :data-testid="`edit-draft-${draft.id}`" @click="beginDraft(draft)">{{ t('agentCatalog.edit') }}</button><button class="record-action" :disabled="saving" :data-testid="`validate-draft-${draft.id}`" @click="validateDraft(draft)">{{ t('agentCatalog.validate') }}</button></footer></article></section>
    </div>
