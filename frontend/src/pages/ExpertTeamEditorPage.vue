...
import { platformApiKey, runtimeEngineDisplayName, type Expert, type ExpertTeam, type ExpertTeamInput, type ModelProviderConnection } from "../api/client";
...
onMounted(async () => {
  try {
    [experts.value, connections.value] = await Promise.all([api.listExperts(), api.listModelProviderConnections()]);
    if (!isNew) {
...
function executionProfile(expert: Expert): string { const model = connections.value.flatMap((connection) => connection.models).find((item) => item.id === expert.provider_model_id); const compatibility = expert.compatibility === "incompatible" ? t("experts.incompatible") : expert.compatibility === "unverified" ? t("settings.unverified") : t("settings.verified"); return `${expert.provider_model_name || model?.display_name || t("experts.modelUnavailable")} · ${runtimeEngineDisplayName(expert.runtime_engine)} · ${compatibility}`; }
</script>
...
<template>
  <section class="page-surface editor-page">
    ...
    <form class="editor-form" @submit.prevent="save">
      ...
      <section class="editor-section"><div><h2>{{ t('experts.members') }}</h2><p>{{ t('experts.membersHint') }}</p></div><div><div class="member-picker"><select v-model="selectedExpertID"><option value="">{{ t('experts.chooseExpert') }}</option><option v-for="item in candidates" :key="item.id" :value="item.id">{{ item.name }} · {{ executionProfile(item) }}</option></select><el-button :icon="Plus" :disabled="!selectedExpertID" @click="addMember">{{ t('experts.add') }}</el-button></div><ol class="ordered-members"><li v-for="(member, index) in members" :key="member.id" draggable="true" @dragstart="draggedIndex = index" @dragend="draggedIndex = undefined" @dragover.prevent @drop="dropMember(index)"><GripVertical :size="17" /><span class="member-order">{{ index + 1 }}</span><div><strong>{{ member.name }}</strong><small>{{ member.capability_introduction }}</small><small>{{ executionProfile(member) }}</small></div><el-button text circle :disabled="index === 0" :aria-label="t('experts.moveUp', { name: member.name })" @click="move(index, -1)"><ArrowUp :size="16" /></el-button><el-button text circle :disabled="index === members.length - 1" :aria-label="t('experts.moveDown', { name: member.name })" @click="move(index, 1)"><ArrowDown :size="16" /></el-button><el-button text circle type="danger" :aria-label="t('experts.removeMember', { name: member.name })" @click="form.expert_ids.splice(index, 1)"><X :size="16" /></el-button></li></ol><el-alert type="info" :closable="false" :title="`${t('experts.perRound', { count: members.length })} · ${t('experts.sequential')}`" /></div></section>
    </form>
  </section>
