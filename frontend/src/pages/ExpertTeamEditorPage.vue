...
<template>
  <section class="page-surface editor-page">
    ...
    <form class="editor-form" @submit.prevent="save">
      <section class="editor-section"><div><h2>{{ t('experts.teamInfo') }}</h2><p>{{ t('experts.teamInfoHint') }}</p></div><div class="form-grid"><label>{{ t('experts.teamName') }}<el-input v-model="form.name" maxlength="100" show-word-limit /></label><label>{{ t('experts.icon') }}<el-select v-model="form.icon"><el-option value="users" :label="`◎ ${t('experts.iconTeam')}`" /><el-option value="sparkles" :label="`✦ ${t('experts.iconSparkles')}`" /><el-option value="compass" :label="`⌖ ${t('experts.iconCompass')}`" /></el-select></label><label>{{ t('experts.iconBackground') }}<el-select v-model="form.icon_background"><el-option value="sage" :label="t('experts.sage')" /><el-option value="sand" :label="t('experts.sand')" /><el-option value="sky" :label="t('experts.sky')" /><el-option value="coral" :label="t('experts.coral')" /></el-select></label><label class="full">{{ t('experts.introduction') }}<el-input v-model="form.introduction" type="textarea" :rows="3" maxlength="2000" show-word-limit /></label><label class="full">{{ t('experts.coreCapability') }}<el-input v-model="form.core_capability" type="textarea" :rows="4" maxlength="20000" show-word-limit /></label></div></section>
