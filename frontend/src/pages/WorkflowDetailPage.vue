...
<template>
  <section class="detail-page workflow-detail-page" :class="{ 'workflow-run-view': selectedRun }">
    ...
    <template v-else>
      ...
      <div v-if="tab === 'workspace'" class="tab-content"><div class="section-heading"><div><p class="eyebrow">PERSISTENT / READ ONLY / {{ Math.round(workspaceUsage.used / 1024 / 1024) }} MB / 1 GB</p><p class="path-line">/{{ workspacePath }}</p></div><span class="engine-chip">{{ t('common.readOnly') }}</span></div><div class="usage-track"><i :style="{ width: `${Math.min(100, workspaceUsage.used / workspaceUsage.limit * 100)}%` }"></i></div><div class="file-browser"><button v-if="workspacePath" class="file-row" @click="loadDirectory(parentPath())"><span class="file-icon">UP</span><strong>..</strong></button><div v-for="entry in entries" :key="entry.path" class="file-row" role="button" tabindex="0" @click="openEntry(entry)" @keydown.enter="openEntry(entry)"><span class="file-icon">{{ entry.directory ? 'DIR' : 'DOC' }}</span><strong>{{ entry.name }}</strong><small>{{ entry.directory ? '—' : `${entry.size} B` }}</small><time>{{ new Date(entry.modified_at).toLocaleString() }}</time><button v-if="!entry.directory" class="text-button" :aria-label="t('common.download')" @click.stop="downloadEntry(entry)">↓</button></div><div v-if="!entries.length" class="empty-inline"><span>□</span><p>{{ t('common.empty') }}</p></div></div></div>
      ...
      <form v-if="tab === 'settings' && !workflow.deleted" class="tab-content settings-form" @submit.prevent="saveSettings">
        ...
        <section class="danger-zone"><h3>{{ t('workflows.deleteTitle') }}</h3><p>{{ t('workflows.deleteDescription') }}</p><button type="button" class="button danger" @click="confirmWorkflowDelete = true">{{ t('common.delete') }}</button></section>
      </form>
      </template>
    </template>
  </section>
