...
async function openArtifact(item: Artifact) { if (item.kind === "file" && !item.expired) { try { const download = await api.getArtifactDownload(workflowID.value, item.id); triggerBrowserDownload(download.url, item.name); } catch { error.value = t("errors.generic"); } } }
