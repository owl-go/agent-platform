...
func TestCreateAppliesIsolationAndResourcePolicy(t *testing.T) {
	...
	for _, required := range [][]string{
		...
		{"--pids-limit", "256"},
		{"--cpus", "2"},
		{"--mount", "type=volume,src=workspace-run-1,dst=/workspace,readonly=false"},
		{"--mount", "type=bind,src=/private/credentials/run-1,dst=/run/agent-credentials,readonly=true"},
