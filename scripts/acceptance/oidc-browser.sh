...
cleanup() {
  ...
  if [[ "$cleanup_status" -ne 0 ]]; then
    [[ -f "$acceptance_tmp/api.log" ]] && cat "$acceptance_tmp/api.log" >&2
    [[ -f "$acceptance_tmp/web.log" ]] && cat "$acceptance_tmp/web.log" >&2
  fi
