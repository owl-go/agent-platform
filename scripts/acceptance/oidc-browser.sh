...
browser --session "$playwright_session" run-code 'async (page) => {
  ...
  const expected = await page.evaluate(async ({ token, runID }) => fetch(`/api/v1/runs/${runID}`, { headers: { Authorization: `Bearer ${token}` } }).then((response) => response.json()), { token: accessToken, runID: values.runID });
  const detail = page.locator(".operations-detail"); const text = await detail.textContent(); if (!text.includes(values.runID) || !text.includes(expected.runtime_image_snapshot.image_digest) || !text.includes(expected.configured_model_snapshot.model_id) || !text.includes("Run Operator denial acceptance")) throw new Error("Operations diagnostics did not render real frozen Run data");
  await page.reload(); await page.getByTestId(`operation-run-${values.runID}`).waitFor(); if (!page.url().includes(`run=${values.runID}`) || !page.url().includes("state=waiting_confirmation")) throw new Error("Operations investigation URL was not restored");
