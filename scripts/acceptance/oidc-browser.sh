...
"$pwcli" --session "$playwright_session" run-code 'async (page) => { await page.getByTestId("sign-in-button").click(); await page.getByRole("textbox", { name: "Username or email" }).fill("platform-user"); await page.getByRole("textbox", { name: "Password", exact: true }).fill("acceptance-only-password"); await page.getByRole("button", { name: "Sign In" }).click(); await page.waitForURL("http://127.0.0.1:18092/"); const user = page.getByTestId("current-user"); if (!(await user.textContent()).includes("Platform User") || !(await user.textContent()).includes("Acme")) throw new Error("real User bootstrap was not rendered"); }'
"$pwcli" --session "$playwright_session" reload
"$pwcli" --session "$playwright_session" run-code 'async (page) => { if (!(await page.getByTestId("current-user").textContent()).includes("Platform User")) throw new Error("OIDC session was not restored after reload"); const localValues = await page.evaluate(() => Object.values(localStorage)); if (localValues.some((value) => /access_token|refresh_token/i.test(value))) throw new Error("OIDC token data was written to localStorage"); await page.getByTestId("sign-out").click(); await page.getByTestId("sign-in-button").waitFor(); if (await page.locator(".shell").count()) throw new Error("protected shell remained visible after logout"); }'
